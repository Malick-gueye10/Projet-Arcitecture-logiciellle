...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if(empty($input_prenom)){
        $prenom_err = "Veillez entrez une prenom.";     
    } else{
        $prenom = $input_prenom;
    }
    ...
    if(empty($input_roles)){
        $roles_err = "Veillez entrez l'roles.";     
    } elseif(!ctype_digit($input_roles)){
        $roles_err = "Veillez entrez une valeur positive.";
    } else{
        $roles = $input_roles;
    }
    
    /* verifier les erreurs avant modification */
    if(empty($name_err) && empty($prenom_err) && empty($roles_err)){
        
        $sql = "UPDATE students SET nom=?, prenom=?, roles=? WHERE id=?";
         
        if($stmt = mysqli_prepare($link, $sql)){
            
            mysqli_stmt_bind_param($stmt, "sssi", $param_nom, $param_prenom, $param_roles, $param_id);
            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post">
                        ...
                        <div class="form-group">
                            <label>prenom</label>
                            <input name="prenom" class="form-control <?php echo (!empty($prenom_err)) ? 'is-invalid' : ''; ?>"><?php echo $prenom; ?></input>
